...
| OBS-2 | runner materializes handoff telemetry | custom fixture driver writes runtime state | run one attempt | canonical `telemetry/handoff-states.json` exists; report aggregates reason/role/lane; old artifacts unavailable | report reaches into mutable runtime state | handoff-observability.test.ts | focused test | aggregate + unavailable |
...
| WASTE-1 | usage joins handoff terminal state | custom usage + FAIL/PASS handoffs | rebuild report | non-PASS rounds/tokens, planner ratio, reopen median, context sequence | transcript inference or zero-round handoff dropped | benchmark-additions.test.ts | focused test | exact joined metrics |
