...
| CLI-3 | unknown args stable non-zero (§13.1) | repo CLI | unknown subcommand/option, missing `--dataset`/`--run`, malformed/unknown `--budget`, malformed `--attempts` | exit 2, stderr names the problem | silent passthrough to a model | cli-surface.test.ts | same | exit 2 + named error |
...
| E2E-2 | schema_version + whole-line writes (§10) | E2E-1 out dir | parse every .json/.jsonl | every JSON document carries its stream schema; every JSONL line parses; no `.tmp` leftovers | half-JSON read as a result after interruption | e2e-offline.test.ts | same | parse + no tmp |
...
| PRED-1 | predictions official contract (§10, §13.8) | E2E-1 predictions | read jsonl | exactly `instance_id`/`model_name_or_path`/`model_patch`, dataset order, one line per selected instance | harness rejects the file | e2e-offline.test.ts, predictions.test.ts | both files | exact key set |
...
| BUD-1 | wall time is the only termination budget; resource counts are observational | module constants | read `DEFAULT_BENCHMARK_BUDGETS`, `CONSUMPTION_METRICS` | `{wall_seconds:5400}` plus four non-terminating metrics | resource caps return | budgets.test.ts | `bun test tests/benchmark/budgets.test.ts` | constants |
...
| EXT-1..6 | the real telemetry-ledger extension stamps rows with the EMITTING assistant context (§6/§7) | real extension + fake pi events, root and delegated env | fire `message_end`/`tool_call`/`tool_execution_end` | rows carry the response's provider/model next to role/depth/goal/thread; a multi-model role attributes each call (and its late result row) to the model that emitted it; orphan calls keep non-empty attribution; usage-less messages stay failed attempts; fed `input.command`/`result.stdout` canaries never serialize | role-level model map misattributes multi-model roles; payload leak at the emitter | tool-attribution.test.ts | same | staging rows |
| RUN-1..3 | the runner writes attributed rows from driver events (§7) | scripted drivers incl. standalone `tool_calls` events | `runBenchmark`, read `cases/<i>/attempts/1/tool-calls.jsonl` | round-attached and standalone calls carry the emitting round's provider/model + role/goal/thread; multi-call/retry/cancelled/incomplete counting unchanged | runner drops attribution or breaks counting | tool-attribution.test.ts | same | per-call rows |
| CHAIN-1 | the PRODUCTION driver forwards staging attribution on tool events (§7) | production `codeflow-driver.ts` + fake inner codeflow (scripted) | spawn the real script, consume events | standalone `tool_calls` events carry the staging rows' provider/model (fake-anthropic/fake-worker); rounds keep theirs | forwarding drops attribution between staging and runner | tool-attribution.test.ts | same | event fields |
| CNT-1..4 | report by-model tool counts equal the RECORDED provider/model grouping, never identity inference (§7, §11) | hand-built out dir with multiple models, zero-tool usage and zero-round tool emitters; fixture run | `buildBenchmarkReport` | exact p/A vs p/B split; usage-only models and tool-only goal/thread/model/depth groups stay visible; fixture report reproduces nonzero ledger grouping | ambiguous or zero-sided dimensions silently dropped; misattributed counts | tool-attribution.test.ts | same | complete by_model/by_goal/by_thread/by_depth totals |
...
| REAL-5 | usage instrumentation feeds ledgers (§6) | REAL-1 usage.jsonl + failed-model-attempts.jsonl | read ledgers | 1 row per completed round with role/provider/model/handoff/goal/thread; failed attempt separate, error_class token only | rounds derived from transcripts; message text persisted | realmode-e2e.test.ts | same | rows + attribution |
...
| REAL-17 | the wall limit kills a LIVE process mid-script; already-streamed rows remain durable | stream-mode fake, 4 scripted rounds, `--budget wall-seconds=1` | CLI run | SIGTERM marker + NO natural-exit marker + fewer events emitted than scripted; at least one usage row at signal time; STEP_1 partial patch still submitted + graded resolved; report counts wall stop | stop discards work, masks as unresolved, or loses durable rows | realmode-streaming.test.ts | same | terminated_by + live observations |
...
## Design B observability additions

| id | criterion | fixture / state | action | expected observable result | boundary / risk | file | runner | signal |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CACHE-1 | prompt-cache rate is token-weighted | hand-built attempts with explicit cache values | rebuild report | suite rate uses summed prompt tokens; unavailable attempt poisons suite availability and is excluded from distribution | averaging attempt rates or treating unreported as zero | report.test.ts, tokens-cache.test.ts | focused test | exact rate + availability |
| OBS-1 | handoff state projection is metadata-only | runtime state files containing prose/receipts | project + canonical read | only closed attribution/status/reason fields survive; extra payload keys rejected | benchmark imports handoff prose | handoff-observability.test.ts | focused test | no prose/extra keys |
| OBS-2 | runner materializes handoff telemetry | custom fixture driver writes runtime state | run one attempt | canonical `telemetry/handoff-states.json` exists; report aggregates reason/role/thread; old artifacts unavailable | report reaches into mutable runtime state | handoff-observability.test.ts | focused test | aggregate + unavailable |
| TIME-1 | source tool timestamps survive the driver | production driver + fake inner ledger | inspect DriverEvent/canonical rows | requested/result source timestamps preserved; incomplete has null result | runner overwrites source time with arrival time | driver-streaming.test.ts, benchmark-additions.test.ts | focused test | timestamp equality |
| TIME-2 | overlapping tool duration is wall-clock union | two overlapping source intervals | rebuild metrics | union duration, not summed durations; TTFP sees untracked files | double-counting parallel tools | benchmark-additions.test.ts | focused test | exact seconds |
| ATTR-1 | usage v2 carries depth/turn/run attribution | real extension event sequence | inspect usage row | schema 2; depth/turn/run/request fields present or explicitly null | B4 joins guess attribution | tool-attribution.test.ts, tokens-cache.test.ts | focused test | exact attribution |
| WASTE-1 | usage joins handoff terminal state | custom usage + FAIL/PASS handoffs | rebuild report | non-PASS rounds/tokens, worker ratio, reopen median, context sequence | transcript inference or zero-round handoff dropped | benchmark-additions.test.ts | focused test | exact joined metrics |
| MULTI-1 | attempts are isolated and non-official | custom evaluator varies by a1/a2 | `attempts: 2` | two workspaces/evaluation ids; one prediction per instance; pass@1/pass@N/dispersion; `not_official` | evaluator cache collision or unofficial result treated as official | benchmark-additions.test.ts | focused test | ids + statistics |
