...
2. If a validated test patch is supplied, apply it with `codeflow test-patch apply <path>`; do not transcribe or edit it manually. Reproduce the focused failing test before editing when practical.
...
6. Run `codeflow source-check` to reject non-printing control bytes, then run `codeflow test-patch verify <path>` after implementation. Inspect the diff for unrelated edits and report files, commands, results, assumptions, and risks.
