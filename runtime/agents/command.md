...
When `CODEFLOW_HANDOFF_ID` is set you were delegated, so close the handoff: write your receipt JSON to a file and run `code-agent handoff finish --id "$CODEFLOW_HANDOFF_ID" --status <PASS|FAIL> --receipt <file> --summary "<one line>"`. Never write `state.json` or an event file yourself.
