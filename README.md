...
Codeflow 是一个 equal-worker 多进程编码工作流，以**单个 skill** 的形式分发。
...
外环（宿主 Agent，读 SKILL.md）
  发起一次 run，然后阻塞等待事件；只看元数据，不进内层上下文
        │
        ▼
内环（equal workers，共享同一模型绑定与 worker prompt）
  root worker
    ├─ 直接完成工作
    └─ 可选打开其他 worker handoff

  goal contract + thread sessions + statistics-only view
```

- **内环**沿用 pi agents 机制与 handoff 语义：每个 worker 是独立进程，由 `runtime/roles.json` 提供统一模型绑定与 worker prompt，通过 handoff 移交工作单元。
...
| `code-agent` | worker 进程 | `delegate` `handoff` `facts` `check` `roster` |
...
铁律：**状态变更与状态查询是程序式的，需求表达与任务编排走模型与提示词。** CLI 独占状态迁移、序号分配、回执校验与事件投递；模型只写 handoff 正文、回执叙述与诊断。任何 worker 都不得手写状态文件。
...
`exec` 创建新 run；`resume` 只续跑一个已经完全停止的既有 run。续跑沿用同一 run id、原始 requirement、root session、goal/thread sessions、facts 和 evidence，并创建下一条 root worker handoff；已经终态的 handoff 不会重开。

只有最新 attempt 已按顺序产生 `run_finished` 和 `runner_exited` 时才能 `resume`，避免同一 run 中出现两个 root worker。该命令只能由人或外环显式调用，不会自动重试，也不能从另一个 Codeflow run 内调用。
...
references/           # worker 提示、方法参考与运行契约
                      # capabilities/ 是全部 project worker 的同一系统提示，不暴露成宿主 skill
                      # usage.md 定义 benchmark 可读取的模型用量报告
tests/                # 每个模块一个目录，各自可独立运行
runtime/              # 内环运行时
├── roles.json        #   worker 注册表：模型、prompt、工具、上下文
...
| 层 | 负责 | 不负责 | 典型内容 |
|---|---|---|---|
| 根 `SKILL.md` | 宿主外环协议 | 内环状态机与 worker 语义 | 如何 `exec`、`sub`、识别 stop signal、何时 audit |
| `scripts/` | 人工运行的环境预检和运维脚本 | worker 编排、模型提示、状态迁移 | `doctor.sh` |
| `references/` | worker 语义与运行契约 | 可执行代码和模型绑定 | capability prompts、handoff、goal、facts、usage |
| `references/capabilities/` | 全部 project worker 共用的唯一系统提示 | 宿主可发现 skill、身份化 agent prompt | worker |
| `runtime/roles.json` | worker 注册、模型与运行策略 | 行为提示词和状态实现 | prompt 路径、tool allowlist、context、internal |
...
runtime/extensions
  -> runtime/lib
  -> runtime/roles.json
  -> references              # 加载 worker prompt；可解析显式 import 指令
```
...
- `quality/` 保持独立，由 CLI 或 worker 显式调用；
- worker registry 指向 `references/capabilities/worker.md`；必要时 prompt 可用 `codeflow:import` 声明额外参考。

避免：

- `lib/` 反向 import `cli/` 或 `extensions/`；
- extension 自己发明第二套 handoff / goal / event 规则；
- `scripts/` 复制 worker、模型或凭证映射；
- `runtime/` 内出现任何嵌套 `SKILL.md`，避免内部能力泄漏成宿主全局 skill；
- CLI adapter 承担状态 invariant；
- worker prompt 描述文件系统权限或实现 CLI 状态机。

### 文件归属规则

1. **人会直接运行的环境检查** 放 `scripts/`。
2. **模型要读的行为提示** 放 `references/capabilities/worker.md`，全部 project worker 共用一份 prompt；共享运行契约放 `references/`。
3. **worker 模型、prompt 路径和运行策略** 只放 `runtime/roles.json`；project worker 不携带身份字段。
...
`scripts/doctor.sh` 不维护第二份 worker 清单；它从 `runtime/models.json`、本地 `runtime/providers.json`（如果存在）和 `runtime/roles.json` 推导 endpoint/凭证影响。worker 或模型改名后 doctor 不需要手工同步。
...
多进程隔离带来一个真实代价：每个 worker 会话都从当前 handoff 与共享事实开始，早期已经查清的事实若未记录，后续 worker 会再搜索一遍。

Codeflow 分两层处理这个问题：每个 worker 从同一份 prompt 启动，必要的额外参考才用 `codeflow:import` 注入并记录哈希；worker 确认过的事实（文件位置、接口签名、既有约定）记入当前 run 的共识区，后续 worker 直接读取而不是重新搜索。后者是执行期的短期上下文，不是跨项目知识库——不依赖外部记忆后端。

worker 可通过 `$PI_CODING_AGENT_DIR` 直接定位 Codeflow 运行时，运行时目录保持只读；`$CODEFLOW_PROJECT_DIR` 与 `$CODEFLOW_EVIDENCE_DIR` 是显式可写工作区。遇到编排自身的异常时可以检查实现，host guard 会终止任何写入或变更命令，避免诊断过程污染正在使用的运行时。
...
`runtime/providers.json` 是本机动态 provider 注册表：它定义 provider ID、协议、模型元数据，以及 base URL/API key 对应的环境变量名。该文件已被 Git 忽略，便于每台机器保持独立路由；真实 URL 和密钥仍只放在 shell 环境或全局 env 文件中，不写入 JSON。未创建该文件时，Codeflow 仅使用仓库内置的 `runtime/models.json`。要让 worker 使用自定义模型，将 `runtime/roles.json` 中的绑定写为 `<provider-id>/<model-id>`。
