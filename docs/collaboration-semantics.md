...
`<run>/watch.ndjson`（`--log` 可改路径），退出时打印一行 `watch_result`，包含
`outcome`、`status`、`summary`、`remaining`、`last_seq`、去重后的 attention 与最后一次
agents 快照，退出码区分 Task 结论、Runtime 中断与待决策（取值见 SKILL）。退出条件是
唯一契约：Task 收口、执行进程消失/身份不符/中断，以及 settled——所有 Agent 都不在执行
而 Task 仍 open，只能由外层推进。settled 不设首轮宽限，已经 settled 正是调用方要的答案；
没有任何 Agent 的 Task 不算 settled，首次派工前启动的 watch 继续等待。`inactive` 默认
只记录不退出（安静不等于死亡），`--wake-on-idle` 才让它唤醒；首个观察周期就已存在的
异常视为继承状态，只产生 attention，重启的观察者不会立刻自杀。`--stay-on-settled`
保留跨 idle 期持有单一进程的旧行为。流式模式行为不变，结尾同样追加 `watch_result`。

宿主优先用后台进程跑 `--quiet`，让进程退出本身成为唤醒信号；流式消费时在程序层处理
空的传输等待，只把有意义的变化交回外层。CLI 本身不保证能唤醒已经结束的宿主对话。观测不启动模型，也不读取 session、
